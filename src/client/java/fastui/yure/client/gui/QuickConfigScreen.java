...
public final class QuickConfigScreen extends Screen {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (FastMasaConfigs.Generic.RELEASE_TO_CLOSE.getBooleanValue() == false && isOpenHotkeyPressedAgain(keyCode)) {
            this.close();
            return true;
        }
    ...
    private void refreshShortcuts() {
        if (this.panelMode == QuickConfigPanel.PanelMode.ENABLED_BOOLEANS) {
            this.items = ConfigIndexService.scanSupportedConfigs().stream()
                    ...
                    .filter(entry -> entry.config() instanceof IConfigBoolean config && config.getBooleanValue())
    ...
    /**
     * 松开关闭被禁用时，完整热键组合再次按下会关闭当前面板。
     * 当前触发的键不一定已进入物理按键状态，因此仅校验组合中其余按键。
     */
    private boolean isOpenHotkeyPressedAgain(int pressedKeyCode) {
        ...
        if (keys.contains(pressedKeyCode) == false) {
            return false;
        }
